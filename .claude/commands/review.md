...
**Prerequisite:** Requires `spec.md` and `design.md` (generated by `/spec` and `/design`). `rfc.md` and `tasks.md` make the review more thorough. If required files are missing from the feature directory, tell the user which commands to run first.
